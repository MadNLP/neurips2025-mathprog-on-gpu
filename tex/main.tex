...
On the other hand, when the matrix is positive (quasi-)definite or close to be positive (quasi-)definite, LDL$^\top$ factorization on GPU often achieve high degree of robustness even without numerical pivoting.
...
Therefore, to fully exploit the benefits of existing GPU direct solvers, it is crucial to develop a \emph{pivoting-free} algorithm that does not rely on dynamic pivoting, which can be achieved by expressing the \gls*{kkt} systems as positive quasi-definite or at least nearly positive quasi-definite matrices.
...
However, in the context of \gls*{nlp}, this system is always \gls*{spd} under the application of primal-dual regularization $(\delta_p, \delta_p)$ chosen based on the standard inertia correction procedure (does not require excessive regularization) \cite{}.
...
\item \textbf{Alternative Strategies}: While we have focused on a plain \gls*{ipm}, the \gls*{ipm} can be combined with other optimization strategies, such as the augmented Lagrangian method, which naturally provides a mechanism to eliminate the indefiniteness of the KKT system via a quadratic penalty term. Augmented Lagrangian or penalty-based approahces may be worth investigating in the context of GPU-accelerated solvers.
...
However, developing hardware-agnostic solvers that can run on a variety of GPU architectures, such as AMD and Intel GPUs, would be highly beneficial.
