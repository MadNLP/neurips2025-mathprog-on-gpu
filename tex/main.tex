...
For \glspl*{lp}, we performed the benchmark on the MIPLIB 2010 benchmark library \cite{kochMIPLIB20102011}, while for \glspl*{nlp}, the benchmark was conducted on the pglib-opf \cite{babaeinejadsarookolaeePowerGridLibrary2021} and the COPS library \cite{dolanBenchmarkingOptimizationSoftware2001}.
...
The benchmark was performed on a workstation with two Intel(R) Xeon(R) Platinum 6130 CPUs @ 2.10GHz, two Quadro GV 100 GPUs (interfaced via PCIe v3.0), and 128 GB of RAM (2400 MT/s).
...
\begin{table} 
  \footnotesize
  \input{table.tex}
  \caption{Solve times of CPU solvers (Gurobi and Ipopt) and GPU solvers (MadIPM and MadNLP) in SGM10 (seconds; with a maximum wall time of 900 seconds) on MIPLIB (88 small, 58 medium, and 28 large \glspl*{lp}), pglib-opf (31 small, 24 medium, and 11 large \glspl*{nlp}), and COPS (13 small, 16 medium, and 16 large \glspl*{nlp}) instances. For Gurobi, Barrrier method is used and Presolve and Crosover options are turned off. MadNLP is configured with cuDSS as the linear solver, while Ipopt is set up with either Ma27 (for pglib-opf) or Ma57 (for COPS), utilizing OpenBLAS as the BLAS and LAPACK backends. All nonlinear models are implemented with ExaModels (running on either GPU or CPU).
}
  \label{tab:results} 
\end{table}
...
We have performed the benchmark on the COPS benchmark library over curated instances. COPS benchmark instances are scalable, in the sense that the user can specify the size of problem. For each type of instance, we have formulated the problem with five different sizes, roughly by doubling the nubmer of variables and constraints.
