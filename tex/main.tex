...
Here, we do not explicitly consider equality constraints for simplicity.
We will primarily discuss \gls*{lp} ($f$ and $g$ are affine) and local solution of \gls*{nlp} ($f$ and $g$ are nonlinear).
We place special emphasis on large, sparse problems in which the constraint Jacobian and Lagrangian Hessian may have millions of rows and columns, yet each row has only tens of nonzero entries.
...
GPU computing is most effective for highly repetitive operations on large data sets, such as dense matrix multiplication (GEMM) routines utilized within large AI model training.
...
In most modern optimization workflows, the derivative evaluation code (or interpreter) is generated in a fully automated fashion via the use of algebraic modeling systems equipped with \gls*{ad} capabilities, such as AMPL, GAMS, CasADi, JuMP, Pyomo, and others.
Thus, it is of interest to develop an algebraic modeling system that can automatically generate the native GPU code for evaluating and differentiating the functions $f$ and $g$.
This is crucial for ensuring the GPU residency of the entire optimization workflow (avoiding data transfer to/from the host memory).

The parallelization of derivative evaluation requires exploiting the internal structure of the functions $f$ and $g$.
We particularly focus on the fac that many practical instances of the large-scale sparse mathematical programs exhibit highly repetitive structure. For example, $f$ may be a sum of many terms (e.g., $f(x) = \sum_{p\in P} f^o(x; p)$) and $g$ may be a collection of many constraints generated from a common template (e.g., $g(x) = \left\{g^o(x; p)\right\}_{p\in P}$).
In such cases, the evaluation and differentiation of $f$ and $g$ is embarassingly parallelizable, and they are suitable for executing on GPUs.

One can develop an algebraic modeling system that auto-generates the derivative evaluation code exploiting this structure, as long as the repetive structure is informed to the modeling system. The emerging algebraic modeling systems, such as ExaModels.jl and Gravity, are desinged to capture such repeated structures. For example, ExaModels.jl requires the user to specify the objective and constraint functions always in a form of iterator, such as
\begin{lstlisting}
...
The classical modeling systems, such as AMPL, GAMS, CasADi, and JuMP, currently do not store the data in such a structured format, and thus, in order to fully leverage the GPU capabilities, the model should be implemented with the GPU-compatible modeling platforms, such as ExaModels.jl.
