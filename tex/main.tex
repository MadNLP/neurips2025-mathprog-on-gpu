...
The \gls*{ipm} is a class of optimization algorithms designed to solve inequality-constrained optimization problems such as \cref{eqn:opt}. These methods transform \cref{eqn:opt} into a sequence of log-barrier subproblems:
...
\begin{center}
  ...
  \input{cops.tex}
\end{center}
The results are largely similar to those of the pglib-opf benchmark, but the speed-up is more pronounced in these instances, as the COPS benchmark library is designed to be scalable, thus exhibiting a highly regular structure, which provides more opportunities for exploiting parallelism. Again, for large instances, one can achieve more than a 10x speed-up on average, while for small and medium-sized instances, the GPU solver is unable to outperform the CPU solver. The speed-up is also substantial for high-precision solves, although the speed-up factor is smaller overall.
