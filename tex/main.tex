...
For nonconvex problems, cuDSS does not currently provide the robust LDL$^\top$ factorization that ensures a symmetric indefinite system can always be factorized.
...
\begin{align}\label{eqn:kkt_system}
  ...
  \end{bmatrix},
\end{align}
where $\mathcal{L}(x,\lambda,s)  := f(x) - \lambda^\top (g(x) - s)$ is the Lagrangian function and $\delta_p,\delta_d\geq 0$ are primal and dural regurlization parameters employed by the \gls*{ipm} solver to ensure the well-posedness of \cref{eqn:kkt_system}.
