...
However, second-order methods heavily relies on direct linear \emph{solvers}, making their adoption on GPUs challenging.
...
Accordignly, the development of second-order solver hinges on the availability of direct linear solvers.

This status quo has change in the recent years as NVIDIA released cuDSS, a direct sparse solver library for GPUs. cuDSS provides robust implementations of direct solvers based on Cholesky and LDL$^\top$, core factorization methods in state-of-the-art mathematical programming solvers. In particular, for several algorithms for convex problems, like LPs and QPs, these capabilities can be directly utilized. For nonconvex problems, while they do not include every component necessary for fully porting IPM solvers, they offer enough functionality to implement modified versions of these algorithms. Not surprisingly, this solver has fueled advances in GPU-accelerated second-order solvers. Several open-source solvers, including MadNLP and Clarabel, already integrate cuDSS in their GPU-accelerated \gls*{nlp} frameworks, demonstrating more than a 10× speedup on large-scale nonlinear problems compared to top CPU-based alternatives \cite{}. 
...
\item We particularly focus on solving KKT systems. Although implementing other algorithmic components, such as pentaly parameter tuning and line serach, requires various low-level kernels, most of them can be implemented through (embarassingly parallel) \texttt{map} or \texttt{reduce} operations. Solving KKT systems, however, involves more advanced algorithms and warrants a deeper discussion.
\item We focus on NVIDIA GPUs and their software stack, because as of today, to the best of our knowledge, they are the only platform that provides adequately mature direct spase solver capabilities.
\end{itemize}
...
% This factorization method is guaranteed to be numerically stable when it is applied to symmetric quasi-definite matrices, and thus, is particularly suitable for factorizing the KKT system matrices encountered within the \gls*{ipm} for convex optimization problems.
...
\end{align}
where $s \in \mathbb{R}^m$ is the slack variable and $\mu > 0$ is the barrier parameter gradually sent to zero. The solution to \cref{eqn:barrier} is sought by attempting to solve the KKT conditions for \eqref{eqn:barrier}:
...
Since the block structure in \cref{eqn:augmentedKKT} admits several possibilities for further reduction, the KKT system can be condensed in several different ways. In the context of GPU implementations, they offer advantages sometimes simply by making the factorization more efficient or sometimes by enabling numerically stable factorization without pivoting.

\begin{itemize}[leftmargin=*,itemsep=0pt,parsep=0pt,partopsep=0pt]
\item \textbf{Partially condensed system.}
By eliminating $d_s = \Lambda^{-1} \left(\mu e - S d_\lambda - S\Lambda e\right)$, the system
\cref{eqn:kkt_system} reduces to the following block $2\times 2$ system:
