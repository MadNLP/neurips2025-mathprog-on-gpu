...
multipliers, and $e$ is the vector of ones.

\Gls*{ipm} solves \cref{eqn:kkt} iteratively using the Newton method. At each iteration, we obtain the
search direction by solving the KKT system:
...
\eqref{eqn:kkt_system} is equivalent to the following $2\times 2$ \emph{augmented KKT system}:
\begin{equation}\label{eqn:augmentedKKT}
  ...
  \end{bmatrix},
\end{equation}
Note that we have introduced in \eqref{eqn:augmentedKKT} a regularization parameter for the primal variables
$\delta_w\geq 0$ and a regularization parameter for the dual variables $\delta_c\geq 0$.
...
Since the block structure in \cref{eqn:augmentedKKT} admits several possibilities for further reduction, the KKT system can be reformulated in several different ways:
...
Compared to \eqref{eqn:augmentedKKT}, the system size is further reduced. However, since the matrix $ $ can be arbitrarily dense, the resulting system can be arbitrarily dense as well, which can significantly increase the computational cost of the linear solver. Furthermore, this elimination strategy suffers from the same potential ill-conditioning issue as in \eqref{eqn:augmentedKKT}. Upon standard inertial correction, this system is always positive definite. Thus, the system can always be Cholesky factorized with static pivoting, even for nonlinear programs.


For GPU solvers for \glspl*{nlp}, \cref{eqn:kkt_primal} is the most suitable formulation and the other formulations are bound to exhibit numerical instabilities. Thus, the currently existing solver MadNLP.jl is based on \cref{eqn:kkt_primal} formulation. To handle equality constraints more efficiently, the equality constraints are relaxed into doubly bounded inequalities with a small gap, called LiftedKKT system strategy.

For strongly convex programs, all four formulations \cref{eqn:kkt_system,eqn:augmentedKKT,eqn:kkt_dual,eqn:kkt_primal} are appropriate, so one may choose the best one based on the sparsity pattern and degree of ill-conditioning of the KKT system matrix. For general quadratic programs, \cref{eqn:kkt_primal} is most suitable, though other formulations, such as \cref{eqn:kkt_system,eqn:augmentedKKT,eqn:kkt_dual} can also be used when proper regularization, pivot perturbation, and/or iterative refinement strategies are employed. Within the existing solver MadIPM, the original KKT system \cref{eqn:kkt_system} is solved with LDL$^\top$ factorization with some primal-dual regularization strategy ensuring the numerical stability of factorization process.
